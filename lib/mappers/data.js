export default function (app) {
    ...
    class DataMapper extends app.MODULE {
        ...
        seekBlockNetwork(id) {
            app.emit("app.data.seek", id);
            this.debug("err", "Must be implemented method dataMapper::seekBlockNetwork");
        }
        ...
        __addToTop(data, blockchain_height) {
            this.top.height = blockchain_height;
        ...
        __addToMain(data, blockchain_height) {
            this.height[data.getId()] = blockchain_height;
            this.chain[blockchain_height] = data.getId();
            if (this.getHeight() < blockchain_height) {
                this.__addToTop(data, blockchain_height);
            }
            this.list[data.getId()] = data;
            app.emit("app.data.new", {
                chain: 'main',
        ...
        _addDataToOrphan(data) {
            ...
            let res = this.__addToOrphan(data);
        ...
        _addDataToBlockChain(data, forse) {
            ...
            let res = this.__addToMain(data, l);
            data.emit();
        ...
        removeOld() {
            ...
            for (let i in sidelist) {
                ...
                if (d.getTime() < removeTime) {
                    ...
                    app.emit("app.data.removeOld", { chain: 'side', dataId: d.getId(), delta: removeTime - d.getTime() });
            ...
            for (let i in orphanlist) {
                ...
                if (d.getTime() < removeTime) {
                    ...
                    app.emit("app.data.removeOld", { chain: 'orphan', dataId: d.getId(), delta: removeTime - d.getTime() });
