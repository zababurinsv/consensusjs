export default function (app) {
    ...
    class AbstractConsensus extends app.MODULE {
        ...
        constructor(consensus_name, consensus_config_field) {
            ...
            app.emit("app.consensus.create", { name: this.consensus_name, field: this.consensus_config_field, config: this.config });
        }
        extendConfig(name) {
        ...
        applyData(peer, data) {
            ...
            if (!data.isImportant())
                return Promise.reject('Invalid data, is not important');
        ...
        inConsensusVersionRange(data) {
            ...
            let height = 0;
            try {
                height = app.dataManager.getDataHeight(data.getId());
